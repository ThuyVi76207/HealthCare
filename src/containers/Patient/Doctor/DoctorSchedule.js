...
class DoctorSchedule extends Component {
    ...
    async componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (this.props.doctorIdFromParent !== prevProps.doctorIdFromParent) {
            ...
            this.setState({
                allAvaliableTimes: res.data ? res.data : []
            })
        }
    }
    ...
    render() {
        let { allDays, allAvaliableTimes } = this.state
        let { language } = this.props
        return (
            <div className="doctor-schedule-container">
                ...
                <div className="all-available-time">
                    ...
                    <div className="time-content">
                        {allAvaliableTimes && allAvaliableTimes.length > 0 ?
                            <>
                                <div className="time-content-btns">
                                    {allAvaliableTimes.map((item, index) => {
                                        let timeDisplay = language === LANGUAGES.VI ?
                                            item.timeTypeData.value_Vi : item.timeTypeData.value_En
                                        return (
                                            <button key={index} className={language === LANGUAGES.VI ? 'btn-vie' : 'btn-en'}>{item.timeDisplay}</button>
                                        )
                                    })
                                    }
                                </div>
